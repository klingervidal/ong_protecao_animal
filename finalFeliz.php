<?php
	...
	$db = new DB();
	//Animais para adoção
	$adotados = $db->pesquisaAnimaisAdotados();
...
<head>
    <?php echo $css->meta; ?>
	<title><?php echo$css->title;?> Final Feliz</title>
	...
	<section id="work" class="wow fadeInUp ptb ptb-sm-80 text-center">
		...
		<div class="container">
			...
			<div class="row container-grid nf-col-3">
				<?php
					if(!isset($adotados) || empty($adotados)){
						echo "Nenhum animal foi adotado.";
					}
						elseif(isset($adotados) && !empty($adotados)){
							foreach($adotados as $key){
								echo
									'
									<div class="nf-item branding design coffee spacing">
										<div class="item-box">
											<a class="cbox-gallary1" href="system/img/produtos/'.$key['foto'].'" title="'.utf8_encode($key['nome']).'">
												<img class="item-container" src="system/img/produtos/'.$key['foto'].'" alt="'.utf8_encode($key['nome']).'" />
												<div class="item-mask">
													<div class="item-caption">
														<h5 class="white">Encontrei um Lar!!!</h5>
														<p class="white">Fui adotado!! :D</p>
													</div>
												</div>
											</a>
										</div>
										<div style="text-align: justify;">
											...
											<span style="color: black;">'.utf8_encode($key['final_feliz']).'</span>
										</div>
				</div>
									';
							}
